  ...
  /* Every download in the app funnels through here. showSaveFilePicker always opens a native save
     ...
     each file" setting is on. Any failure — the API being missing (Firefox, Safari), a headless
     engine, or the browser rejecting the call — falls back to the classic anchor click below, and
     that includes a rejection named AbortError. It is tempting to treat that one specially and
     stop rather than fall back, on the theory that it means the user clicked Cancel in a dialog
     they saw. It doesn't reliably mean that: Chromium raises the identical AbortError, with the
     identical message, when the call is made without a fresh enough user gesture to grant it —
     which several call sites here do, because they render an image or await a fetch before ever
     reaching this function. Measured directly against this app's own served page: a call made
     without that gesture rejects with AbortError in under a millisecond, indistinguishable from a
     real cancellation. Treating it as terminal made every one of those calls silently produce no
     download at all, with nothing to tell the person who clicked why nothing happened — worse than
     the one real cost of falling back unconditionally, which is that an explicit Cancel saves the
     file anyway instead of discarding it. */
  async function downloadBlob(contents, type, filename) {
    ...
    if (typeof showSaveFilePicker === 'function') {
      try {
        ...
        /* Falls through to the classic path below whatever the error was. */
      }
    }

...
async function downloadPng() {
  ...
  images.forEach((img, i) => {
    ...
    /* The report is a standalone document with no Figure font control of its own to read (the
       main app's figureFont() reads a select that only exists there) — a fixed default matches
       what the main app itself defaults to. */
    ctx.font = '700 ' + Math.round(cap * 0.46) + 'px Arial, Helvetica, sans-serif';
    ctx.fillText(String.fromCharCode(65 + i), x + Math.round(w * 0.025), y + h + cap / 2);
    ctx.font = '500 ' + Math.round(cap * 0.34) + 'px Arial, Helvetica, sans-serif';

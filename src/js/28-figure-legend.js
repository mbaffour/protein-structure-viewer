  ...
  function figureLegendText() {
    ...
    if (alignmentResults.length) sentences.push('Models were superposed on ' + alignmentResults[0].reference + ' by ' + alignmentResults[0].method + ' (' + alignmentResults.map(result => result.name + ': ' + result.count + ' Cα pairs, RMSD ' + result.rmsd.toFixed(2) + ' Å').join('; ') + ').');
  ...
  function methodsText() {
    ...
    if (alignmentResults.length) {
      ...
      sentences.push('Models were superposed on ' + alignmentResults[0].reference + ' by least-squares (Kabsch) fitting of Cα atoms paired by ' + (bySequence ? 'global Needleman–Wunsch alignment of each chain pair (match +2, mismatch −1, gap −2), chains being matched greedily by sequence identity' : 'chain and residue identifier') + '; RMSD is reported over all paired Cα atoms without outlier rejection (' + alignmentResults.map(result => result.name + ': n = ' + result.count + ', ' + result.rmsd.toFixed(2) + ' Å').join('; ') + ').');
